...
function click_handler() {
  ...
  switch (action) {
    case 'upvote':
      err_msg =
        "Bu fantastik derdi sikebilmek için önce dert.gg'ye giriş yapmalısın.";
      break;
    case 'unvote':
      err_msg =
        "Bu derdi sikmekten vazgeçtin anlıyoruz ama önce dert.gg'ye giriş yapmalısın.";
      break;
  }

  if (topic && entry_id) {
    chrome.runtime.sendMessage(
      { type: action, entryId: entry_id, topic: topic },
      (resp) => {
...
function append_dert_gg() {
  ...
  let feedback_container =
    entry_list.getElementsByClassName('feedback-container');
